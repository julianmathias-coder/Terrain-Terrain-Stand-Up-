...
void loop() {
  if (startCountdown.isClicked()) {
    ...
    if (!nfcScanned) {
      ...
      wemoWrite(MYWEMOFan,HIGH); //turn on fan
      wemoWrite(MYWEMOLight,LOW); //turn off salt lamp
      Serial.printf("Turning on Fan %i\n",MYWEMOFan);
      Serial.printf("Turning off Salt Lamp %i\n",MYWEMOLight);
      }  
    }
  }
  ...
  if (nfc.scan()) {
    ...
    wemoWrite(MYWEMOLight,HIGH); //turn on salt lamp
    Serial.printf("Turning off Fan %i\n",MYWEMOFan);
    Serial.printf("Turning on Salt Lamp %i\n",MYWEMOLight);
...
void displayNFCData() {
  for (litPixel=0; litPixel<1; litPixel++) {
    pixel.setPixelColor (litPixel, 0, 255, 0); //RGB Order: Green
    pixel.show();
    pixel.clear();  
  }
